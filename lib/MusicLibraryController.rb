class MusicLibraryController
  ...
  def list_artists
    artists = Artist.all
    alph_list = artists.sort_by! { |artist| artist.name }
    alph_list.each_with_index { |artist, index| puts "#{index+1}. #{artist.name}" }
  end

  def list_genres
    genres = Genre.all
    alph_list = genres.sort_by! { |genre| genre.name }
    alph_list.each_with_index { |genre, index| puts "#{index+1}. #{genre.name}" }
  end
